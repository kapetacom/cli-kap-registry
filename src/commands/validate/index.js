...
module.exports = async function link(source) {
    ...
    if (sourceObject.kind.startsWith('core/')) {
        ...
        if (errors.length === 0) {
            ...
            const provider = ClusterConfiguration.getProviderDefinitions().find((provider) => {
                if (provider.definition.metadata.name === kindUri.fullName &&
            ...
            if (provider.definition?.spec?.schema) {
                errors = validateSchema(provider.definition.spec.schema, sourceObject.spec);
            }
        }
    }

    if (errors.length > 0) {
        cli.error(`File contained errors:\n  - ${errors.map((error) => {
